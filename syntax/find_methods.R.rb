@@find_methods_rcode=<<RCODEFINDMETHODS
	...
	get_pkg_names<-function(pkgname, S3Methods=FALSE){
		...
		if(class(pkgnamespace)=="try-error"){ 
			if(!is.na(installed.packages()[pkgname,'Priority']) && (installed.packages()[pkgname,'Priority']=="base")) {
		...
		pkgmethods<-if(is.matrix(pkgmethods)&&NCOL(pkgmethods)==3) pkgmethods[,3] else pkgmethods
